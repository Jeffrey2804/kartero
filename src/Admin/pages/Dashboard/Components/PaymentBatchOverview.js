...
import PaymentIndividualOverview from './PaymentIndividualOverview';
...
export default function EnhancedTable(props) {
  ...
  const viewBatchOverview = () => {
    adminReportsService
      .getPaymentBatchOverview(props.selectedStatemendId)
      .then((response) => {
        setPaymentBatchOverviewData(response);
        props.setPaymentBatchOverviewData(response);
  ...
  const handleSelectAllClick = (event) => {
    if (event.target.checked) {
      const newSelected = paymentBatchOverviewData.map((n) => n.name);
      setSelected(newSelected);
      return;
    }
    setSelected([showIndividualPayment]);
  };
  ...
  const emptyRows =
    page > 0 ? Math.max(0, (1 + page) * rowsPerPage - paymentBatchOverviewData.length) : 0;
  ...
  return (
    ...
    <Box sx={{ width: '100%' }} >
      ...
      {showIndividualPayment ? <PaymentIndividualOverview
        ...
        <Paper sx={{ width: '70.5vw', mb: 2 }} >
          ...
          >
            ...
            <TableContainer>
              ...
              >
                <EnhancedTableHead
                  ...
                  rowCount={paymentBatchOverviewData.length}
                />
                ...
                <TableBody>
                 ...
                 rows.slice().sort(getComparator(order, orderBy)) */}
                  {stableSort(paymentBatchOverviewData, getComparator(order, orderBy))
                      ...
                      return (
                        ...
                        >
                          ...
                          >
                            ...
                            {row.billingCycleFrom + " - " + row.billingCycleTo

                            }
                          </TableCell>

                          <TableCell align="center">{row.uploadDate.substring(0, 10)}
                          </TableCell>
                          <TableCell align="center">{row.service}</TableCell>
                          <TableCell align="center">{row.statementsSent}</TableCell>
                          <TableCell align="right">{row.statementsPaid}</TableCell>
                          <TableCell align="left">{row.amountCollected}</TableCell>
                          <TableCell align="left">{row.amountUncollected}</TableCell>
                          <TableCell align="center">{row.amountBounced}</TableCell>
                        </TableRow>
                      );
                    })}
                  {emptyRows > 0 && (
            ...
            <TablePagination
              ...
              component="div"
              count={paymentBatchOverviewData.length}
